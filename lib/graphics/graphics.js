...
var Manual = require('../layout/Manual');
var R = require('ramda');
...
function Graphics ( network, settings, state ) {
    ...
    linkRenderer = function ( link ) {
        var from = nodeUIs[link.from.substring(2)];
        var to = nodeUIs[link.to.substring(2)];
    ...
    function initialize () {
        ...
        if ( settings.layout === 'Fixed' ) {
            ...
            linkRenderer = function (ui) {
                var ui1 = nodeUIs[ui.from.substring(2)]; // TODO: Fix IDs!!
                var ui2 = nodeUIs[ui.to.substring(2)];
                var from = ui1.pos;
                var to = ui2.pos;
    ...
    function createLayout ( network, settings ) {
        ...
        if ( typeof input === 'string' ) {
            ...
            if ( input === 'Manual' ) {
                return new Manual(network, settings);
            }
    ...
    function createNodeUI(node) {
        ...
        if (!ui) {
            ...
            ui.z = layers.indexOf('l' + node.data['layer']) * settings.interLayerDistance;
    ...
    function renderFrame() {
        ...
        Object.keys(nodeUIs).forEach(function (id) {
            nodeRenderer(nodeUIs[id])
        });
        Object.keys(linkUIs).forEach(function (id) {
            linkRenderer(linkUIs[id])
        });
