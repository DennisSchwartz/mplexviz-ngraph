...
var Graphics = function (graph, layout, settings) {
    ...
    function init() {
        ...
        console.log(API.layout.simulator);
    ...
    function initNode(node) {
        ...
        if (!ui) return;
        ui.pos = layout.getNodePosition(node.get('node').get('id')); // --> this could probably be done in the UIBuilder
        ui.pos.z = graph.get('layers').indexOf(graph.get('layers').get(node.get('layer'))) * layout.getInterLayerDistance();
        console.log(ui.pos.z);
        console.log(ui);
        nodeUI[id] = ui;
    ...
    function initEdge(edge) {
        ...
        if (!ui) return;
        ui.from = nodeUI[from].pos; //layout.getNodePosition(from);
        ui.to = nodeUI[to].pos; //layout.getNodePosition(to);// --> this could probably be done in the UIBuilder
        edgeUI[id] = ui;
        library.addToScene(ui);
    }
    ...
    function nodeUIBuilder() {
        var nodeMaterial = new library.lib.MeshBasicMaterial( { color: 0x00cccc });
