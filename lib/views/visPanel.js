    ...
    renderGraph: function () {
        ...
        this.el.addEventListener('mousemove', function(event) {
            ...
            if (intersects.length > 0) {
                var curr = intersects[0];
                if (curr.object.geometry.type === 'SphereGeometry') {
                    console.log(curr);
                    scene.children.forEach(function (node) {
                        console.log(node);
                        if (node.geometry.type === 'SphereGeometry') {
                            node.material.color.setHex('0x' + node.myColor.toString(16));
                        }
                    });
                    var color = curr.object.material.color.getHexString();
                    //console.log(shadeBlend(0.3, '#' + color.toUpperCase()).substr(1));
                    curr.object.material.color.setHex('0x' + shadeBlend(0.5, '#' + color.toUpperCase()).substr(1));
                }
            }
        }, false);
