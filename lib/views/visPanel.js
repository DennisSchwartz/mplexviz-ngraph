            ...
            if (intersects.length > 0) {
                ...
                if (INTERSECTED != intersects[0].object) {
                    ...
                    if (INTERSECTED.type === 'Line') {
                        ...
                        object = graph.get('edges').get(INTERSECTED.myId);//graph.getLink(nodes[0], nodes[1]).data;
                        console.log(object);
                        type = 'Edge';
                        name = object.get('label')? object.get('label') : object.get('id');

                    }  else if (INTERSECTED.type === 'Mesh') {
                        INTERSECTED.scale.set( 1.2, 1.2, 1.2 );
                        var id = INTERSECTED.myId;
                        object = graph.get('nodes').get(id);
                        var node = object; //TODO Do a proper refactor!
                        type = 'Node';
                        name = node.get('name') ? node.get('name') : node.get('id');
                        layer = node.get('layer').get('label')? node.get('layer').get('label') : node.get('layer').get('id');
    ...
    applySettings: function (graphics, settings, callback) {
        ...
        graphics.setNodeUI(function (node) {
            ...
            if (settings.colorsByLayers) {
                color_ = nodeColors[layers.indexOf(node.get('layer'))]
            };
