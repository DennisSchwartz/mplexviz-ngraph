...
var nthree = require('../graphics/graphics');
...
var VisPanel = Backbone.View.extend({
    initialize: function (opts) {
        ...
        this.layout = opts.options.layout || 'Fixed';  //mplexLayout(this.graph);
    ...
    renderGraph: function () {
        ...
        this.graphics = nthree(this.graph, {
            ...
            alpha: true,
            antialias: true,
            interLayerDistance: this.settings.interLayerDistance
        });
        ...
        console.log(this.graphics.nodeRenderer);
        ...
        this.el.addEventListener('mousemove', function(event) {
            ...
            if (intersects.length > 0) {
                ...
                if (INTERSECTED != intersects[0].object) {
                    ...
                    /*
                        This is where the selected/hovered node gets highlighted
                     */
                    if (INTERSECTED) {
                    ...
                    INTERSECTED.material.color.setHex( '0x' + shadeBlend(-0.2, '#' +
                    ...
                    /*
                        Here, the  information about the highlighted node is sent to the InfoPanel
                     */
                    var object, type, name, layer;
                    if (INTERSECTED.type === 'Line') {
                        var nodes = INTERSECTED.myId.split('nl');
                        var edgeID = 'e' + nodes[1] + nodes[2];
                        object = graph.elements[edgeID];//graph.getLink(nodes[0], nodes[1]).data;
                        type = 'Edge';
                        name = object.data['label'] ? object.data['label'] : object.data['id'];

                    }  else if (INTERSECTED.type === 'Mesh') {
                        INTERSECTED.scale.set( 1.2, 1.2, 1.2 );
                        var id = INTERSECTED.myId;
                        object = graph.elements['nl' + id];
                        var node = object; //TODO Do a proper refactor!
                        type = 'Node';
                        name = node.data['label'] ? node.data['label'] : node.data['id'];
                        layer = node.data['layer'];

                    }

                    evg.trigger("info:update", {
                        type: type,
                        name: name,
                        layer: layer,
                        activeObject: object
    ...
    updateGraph: function (evt) {
        if (evt.target.id === 'options') {
            ...
            this.applySettings(this.graphics, this.settings, function (graphics) {
                console.log("Trying to reset stable");
    ...
    applySettings: function (graphics, settings, callback) {
        ...
        var layers = this.graph.get('layers');
        //console.log(layers);
        ...
        graphics.setNodeUI(function (node) {
            ...
            var color_ = 0x000000;
            //console.log(node.get('layer'));
            settings.colorsByLayers = true; // Todo: Add this to the real settings
            if (settings.colorsByLayers) {
                color_ = nodeColors[R.map(R.path(['data', 'id']), layers)
                    .indexOf('l' + node.data['layer'])];
            }
            ...
            sphere.myId = node.data['id'];
            sphere.isect = true;
            return sphere;
        });
        graphics.setLinkUI(function (link) {
